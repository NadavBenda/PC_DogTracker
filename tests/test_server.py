...
def test_detections_endpoint_mode_single_is_the_default(client):
    c, _, _ = client
    assert c.get("/api/detections").get_json() == c.get("/api/detections?mode=single").get_json()


def test_detections_endpoint_multi_mode_falls_back_to_single_before_detect_is_called(client):
    """Before /api/multi-dog-detect has ever run, mode=multi must not crash --
    it just hasn't got anything else to serve yet."""
    c, _, dets = client
    data = c.get("/api/detections?mode=multi").get_json()
    assert len(data) == len(dets)


def test_multi_dog_detect_endpoint_computes_and_is_used_by_mode_multi(client, monkeypatch):
    c, frames, _ = client
    multi_dets = [
        Detection(frames[0].filename, frames[0].timestamp_ms, frames[0].width, frames[0].height, 1, 1, 5, 5, 0.9),
        Detection(frames[0].filename, frames[0].timestamp_ms, frames[0].width, frames[0].height, 40, 40, 5, 5, 0.7),
    ]
    calls = []

    def fake_run_multi_dog_detection(folder, frames, rotate_degrees=0):
        calls.append(1)
        return multi_dets

    monkeypatch.setattr("dogtracker_pc.server.run_multi_dog_detection", fake_run_multi_dog_detection)

    res = c.post("/api/multi-dog-detect")
    assert res.status_code == 200
    body = res.get_json()
    assert body["detection_count"] == 2
    assert body["frames_with_multiple"] == 1

    data = c.get("/api/detections?mode=multi").get_json()
    assert len(data) == 2

    # Idempotent: calling again must not re-run detection.
    c.post("/api/multi-dog-detect")
    assert len(calls) == 1


def test_areas_and_visits_and_heatmap_respect_mode(client, monkeypatch):
    c, frames, _ = client
    multi_dets = [
        Detection(frames[0].filename, frames[0].timestamp_ms, frames[0].width, frames[0].height, 1, 1, 5, 5, 0.9),
        Detection(frames[1].filename, frames[1].timestamp_ms, frames[1].width, frames[1].height, 1, 1, 5, 5, 0.9),
    ]
    monkeypatch.setattr(
        "dogtracker_pc.server.run_multi_dog_detection",
        lambda folder, frames, rotate_degrees=0: multi_dets,
    )
    c.post("/api/multi-dog-detect")

    visits = c.get("/api/visits?mode=multi&distance=1000&gap=100000").get_json()
    assert sum(v["frame_count"] for v in visits) == len(multi_dets)

    areas = c.get("/api/areas?mode=multi&distance=1000&gap=100000&area_radius=1000").get_json()
    assert sum(a["visit_count"] for a in areas["areas"]) == len(visits)

    assert c.get("/api/heatmap.png?mode=multi").status_code == 200

    summary = c.get("/api/summary?mode=multi").get_json()
    assert summary["detection_count"] == len(multi_dets)
